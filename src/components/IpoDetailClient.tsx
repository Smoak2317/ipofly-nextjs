...
// Inline compact chart component for better mobile optimization
function CompactGmpChart({ history, isPositive }: { history: any[], isPositive: boolean }) {
  ...
  // Simple line chart using SVG for better performance and control
  const maxGmp = Math.max(...chartData.map(d => Math.abs(d.gmp)));
  const minGmp = Math.min(...chartData.map(d => d.gmp));
  const range = maxGmp - minGmp || 1;
  ...
  const points = chartData.map((d, i) => {
    ...
    const y = height - padding - ((d.gmp - minGmp) / range) * (height - 2 * padding);
    return `${x},${y}`;
  }).join(' ');
  ...
  return (
    <div className="w-full h-full">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full">
        {/* Grid lines */}
        <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke="currentColor" strokeWidth="0.5" className="text-gray-300 dark:text-gray-600" />
        <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke="currentColor" strokeWidth="0.5" className="text-gray-300 dark:text-gray-600" />
